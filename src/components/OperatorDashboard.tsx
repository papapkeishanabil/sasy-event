...
const OperatorDashboard: React.FC<OperatorDashboardProps> = ({ guests, onCheckIn, onUndoCheckIn, onBack }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      console.log('Scroll:', window.scrollY, 'Show button:', showButton);
    };
  ...
  return (
    <div className="page-transition min-h-screen flex flex-col bg-sasie-cream relative">
      {/* Compact Header */}
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-20 right-3 w-10 h-10 rounded-full bg-sasie-gold/90 backdrop-blur-sm shadow-md flex items-center justify-center z-[9999] hover:bg-sasie-gold transition-all duration-200"
          aria-label="Scroll to top"
        >
          <svg className="w-5 h-5" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth={2.5} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 10l7-7m0 0l7 7m-7-7h18" />
          </svg>
        </button>
      )}
    </div>
